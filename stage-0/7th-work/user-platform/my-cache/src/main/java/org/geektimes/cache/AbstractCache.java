...
public abstract class AbstractCache<K, V> implements Cache<K, V> {
    ...
    private final CacheEntryEventPublisher entryEventPublisher; // 获取 缓存对象事件发布器
    ...
    public void registerCacheEntryListener(CacheEntryListenerConfiguration<K, V> cacheEntryListenerConfiguration) {
        // 调用入口： cache.registerCacheEntryListener(cacheEntryListenerConfiguration(new TestCacheEntryListener<>()));
        // 最终在 缓存对象事件发布器（CacheEntryEventPublisher）中添加 事件监听器（CacheEntryEventListenerAdapter），目的便于统一管理操作
        // 事件监听器： 由 TestCacheEntryListener 构建 -> cacheEntryListenerConfiguration 构建 -> CacheEntryEventListenerAdapter 构建
    ...
    private Entry<K, V> updateEntry(K key, V value) {
        ...
        /*
         0.当缓存操作完成后，调用父类（AbstractCache）中 缓存对象事件发布器（CacheEntryEventPublisher），并传入参数 key, oldValue, value
         1.根据 参数 和 指定类型，实例化事件源（eventObject） 通过该方法：org.geektimes.cache.event.GenericCacheEntryEvent.updatedEvent
         2.将 事件源（GenericCacheEntryEvent）作为参数，执行发布器中 publish 方法， 并指定类型 通过该方法：entryEventPublisher.publish(removedEvent(this, key, oldValue));
         3.publish方法执行： 提前注册的eventListener的onEvent方法，通过该方法：listener.onEvent（事件监听器对事件源操作的入口）
         4.onEvent实际执行类： CacheEntryEventListenerAdapter<K, V> implements ConditionalCacheEntryEventListener<K, V>。不会出现执行非指定类型事件处理，进行类型（EventType）过滤
         5.最终通过反射执行 TestCacheEntryListener中(onCreated onExpired onRemoved onUpdated)方法
        */
