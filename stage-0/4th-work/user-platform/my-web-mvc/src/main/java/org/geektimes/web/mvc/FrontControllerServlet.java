...
//                        String viewPath = pageController.execute(request, response);
                        ...
                        String controllerName = handlerMethodInfo.getControllerName();
                        Class<?> klass = Class.forName(controllerName);
                        String viewPath = (String) handlerMethod.invoke(klass.newInstance(), request, response);
                        ...
                        request.setAttribute(CONFIG_NAME, config);

                        // 2.
                        Object attribute = servletContext.getAttribute(CONFIG_NAME);
                        request.setAttribute(CONFIG_NAME, attribute);

                        Object invokeResult = handlerMethod.invoke(klass.newInstance(), request, response);
                        Object castResult = returnType.cast(invokeResult);

                        response.setContentType("application/json;charset=utf-8");
                        response.setCharacterEncoding("UTF-8");

                        PrintWriter out = response.getWriter();
                        out.write(JSON.toJSONString(castResult));
                        out.flush();
                        out.close();
                    }
                }
