...
public class CachingTest {
    ...
    public void testSampleRedis() {
        ...
        // configure the cache
        MutableConfiguration<String, Long> config =
                new MutableConfiguration<String, Long>()
                        .setTypes(String.class, Long.class);

        // create the cache
        Cache<String, Long> cache = cacheManager.createCache("redisCache", config);
    ...
    @Test
    public void testMyLettuce() {
        CachingProvider cachingProvider = Caching.getCachingProvider();
        CacheManager cacheManager = cachingProvider.getCacheManager(URI.create("redis://127.0.0.1:6379/"), null);
        // configure the cache
        MutableConfiguration<String, JSONObject> config =
                new MutableConfiguration<String, JSONObject>()
                        .setTypes(String.class, JSONObject.class);

        // create the cache
        Cache<String, JSONObject> cache = cacheManager.createCache("redisCache", config);
        try {
            // add listener
            cache.registerCacheEntryListener(cacheEntryListenerConfiguration(new TestCacheEntryListener<>()));

            // cache operations
            String key = "redis-ket";
//            List<String> value1 = new ArrayList<>();
//            value1.add("1");
//            value1.add("2");
            JSONObject value1 = new JSONObject();
            value1.put("name", "xiaomage");
            value1.put("age", "35");
            cache.put(key, value1);

            // update
            value1.put("city", "shanghai");
            cache.put(key, value1);

            JSONObject jsonObject = cache.get(key);
